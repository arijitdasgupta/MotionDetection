...
while True: #Main loop
    ...
    #Image processing
    if (motion_detection_skipper > 0):
        motion_detection_skipper = motion_detection_skipper - 1
    else:
    ...
    if avg > param1:
        ...
        rotation_check(x)
        avg = 0
        motion_detection_skipper = motion_detection_skip_limit
        cv.AbsDiff(accumulator, accumulator, accumulator)
    elif avg < -param1:
        print "Movement left"
        serial_port.write('L')
        x = serial_port.read()
